...
int Shell::system(std::string command, std::string cwd, std::string outFile, std::string errFile, int verbosity,
                  RunStatistics* stats) {
...
int Shell::system(const SystemOptions& options, RunStatistics* stats) {
    ...
    PushD dir(realCWD);
    if(messageFormatter)
        messageFormatter->reportAction("Entering directory: `" + realCWD + "'",
                                       MessageFormatter::MessageClass(options.verbosity));

    // Execute the command
    int result = 0;
    if(messageFormatter)
        messageFormatter->reportAction("Executing: " + command, MessageFormatter::MessageClass(options.verbosity));
    ...
    if(stats) {
        ...
        if(messageFormatter)
            messageFormatter->reportAction("Read time statistics (" + statsProgramHandler->getName() + ")",
                                           MessageFormatter::MessageClass(options.verbosity));
    }
    ...
    dir.popd();
    if(messageFormatter)
        messageFormatter->reportAction("Exiting directory: `" + realCWD + "'",
                                       MessageFormatter::MessageClass(options.verbosity));
...
bool Shell::readMemtimeStatistics(const char* contents, RunStatistics& stats) {
    ...
    int result = sscanf(contents, "%f user, %f system, %f elapsed -- Max VSize = %fKB, Max RSS = %fKB",
                        &stats.time_user, &stats.time_system, &stats.time_elapsed, &stats.mem_virtual,
                        &stats.mem_resident
    );
    return result != 5;
}
